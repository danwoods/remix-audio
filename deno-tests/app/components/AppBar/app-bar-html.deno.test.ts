 ...
 *
 * Verifies that the upload button (upload-dialog) is only present when the
...
Deno.test("appBarHtml does not include upload button when isAdmin is false", () => {
   ...
   * contain the upload-dialog element so the upload button is not visible.
   */
  ...
  assert(
    !html.includes("upload-dialog"),
    "Expected app bar HTML to omit upload-dialog when isAdmin is false, but it was present",
  );
});
